...
use crate::transport::{Transport, TransportSnapshot, TransportStatus};
...
impl Server {
    pub fn new() -> Result<Self> {
        ...
        master_runtime_builder.max_blocking_threads(
            proxy_server_config.max_blocking_threads().with_context(|| "Can not get max blocking threads number from proxy configuration.")?,
        );
        ...
        self.master_runtime.spawn(async move {
            loop {
                ...
                match transport_snapshot {
                    ...
                    Some(snapshot) => {
                        info!("Transport status change happen: [{}]", snapshot.id);
                    }
                }
            }
        });
