...
impl Transport {
    pub fn new(agent_remote_address: SocketAddr, snapshot_sender: Sender<TransportSnapshot>) -> Result<Self> {
        Ok(Self {
            id: generate_uuid(),
    ...
    async fn init(&mut self, mut agent_stream_framed: AgentStreamFramed) -> Result<Option<InitResult>> {
        if self.status != TransportStatus::New {
            return Err(PpaassProxyError::InvalidTcpTransportStatus(self.id.clone(), TransportStatus::New, self.status).into());
        }
